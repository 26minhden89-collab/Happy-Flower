...
import { Plus, ArrowUpCircle, ArrowDownCircle, Search, Calendar, Filter, Trash2, Save, X, Boxes, Wallet, ShoppingBag, DollarSign } from 'lucide-react';
import { useFinance } from '../contexts/FinanceContext';
import { useInventory } from '../contexts/InventoryContext';
import { TransactionCategory, UsedMaterial, TransactionType } from '../types';
...
const FinanceList = () => {
  ...
  const [modalType, setModalType] = useState<TransactionType>('EXPENSE'); // Default to Expense
  ...
  const handleOpenModal = (type: TransactionType) => {
    ...
    setFormData({
      date: new Date().toISOString().split('T')[0],
      amount: '',
      category: type === 'INCOME' ? 'SALES' : 'MATERIAL_PURCHASE',
      description: ''
    });
    setDraftMaterials([]);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (modalType === 'EXPENSE' && formData.category === 'MATERIAL_PURCHASE' && draftMaterials.length === 0) {
        alert("Vui lòng thêm ít nhất một vật tư vào danh sách");
        return;
    }

    // LOGIC CHI MUA VẬT TƯ: CỘNG KHO
    if (modalType === 'EXPENSE' && formData.category === 'MATERIAL_PURCHASE') {
  ...
  return (
    <div className="p-6 relative animate-fade-in pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Sổ Quỹ (Thu - Chi)</h1>
          <p className="text-sm text-gray-500 mt-1">Ghi nhận dòng tiền thực tế. Nguồn dữ liệu duy nhất cho báo cáo tài chính.</p>
        </div>
        <div className="flex gap-3">
          <button 
            onClick={() => handleOpenModal('INCOME')}
            className="flex items-center px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 shadow-sm transition-colors"
          >
      ...
      {/* Stats Cards - PURELY FINANCIAL */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
         <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
            <div>
                <p className="text-sm text-gray-500 mb-1">Tổng Thu (Sổ quỹ)</p>
                <div className="text-xs text-gray-400 mb-1">(Tất cả phiếu thu)</div>
                <h3 className="text-2xl font-bold text-green-600">{totalIncome.toLocaleString('vi-VN')} đ</h3>
            </div>
         ...
         <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
            <div>
                <p className="text-sm text-gray-500 mb-1">Tổng Chi (Sổ quỹ)</p>
                <div className="text-xs text-gray-400 mb-1">(Tất cả phiếu chi)</div>
         ...
         <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
            <div>
                <p className="text-sm text-gray-500 mb-1">Quỹ hiện tại</p>
                <div className="text-xs text-gray-400 mb-1">(Thu - Chi)</div>
                        ...
                        <td className="px-6 py-4">
                            {t.type === 'INCOME' ? (
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                    <ArrowUpCircle className="w-3 h-3 mr-1" /> Thu
                                </span>
                            ) : (
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                    <ArrowDownCircle className="w-3 h-3 mr-1" /> Chi
                                </span>
                            )}
                        </td>
                         <td className="px-6 py-4 text-xs font-medium text-gray-500 uppercase">
                             {t.category === 'MATERIAL_PURCHASE' ? 'Mua vật tư' : 
                              t.category === 'SALES' ? 'Bán hàng' : 'Khác'}
                         </td>
                        <td className="px-6 py-4">
      ...
      {isModalOpen && (
          <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
              <div className="bg-white rounded-xl w-full max-w-[550px] shadow-2xl flex flex-col max-h-[90vh]">
                 {/* Header - Fixed */}
                 <div className={`px-6 py-4 border-b flex justify-between items-center rounded-t-xl shrink-0 ${modalType === 'INCOME' ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}>
                    <h3 className={`text-lg font-bold flex items-center ${modalType === 'INCOME' ? 'text-green-800' : 'text-red-800'}`}>
                        {modalType === 'INCOME' ? <ArrowUpCircle className="w-5 h-5 mr-2" /> : <ArrowDownCircle className="w-5 h-5 mr-2" />}
                        {modalType === 'INCOME' ? 'Tạo phiếu Thu' : 'Tạo phiếu Chi'}
                    </h3>
                 ...
                 <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                     <form id="transactionForm" onSubmit={handleSave} className="space-y-4">
                        
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Ngày giao dịch</label>
                                <input 
                                    type="date" 
                                    required
                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 text-sm ${modalType === 'INCOME' ? 'focus:ring-green-500' : 'focus:ring-red-500'}`}
                                    value={formData.date}
                                    onChange={e => setFormData({...formData, date: e.target.value})}
                                />
                            </div>
                            <div>
                                 <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Hạng mục</label>
                                 <select 
                                    className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 text-sm bg-white ${modalType === 'INCOME' ? 'focus:ring-green-500' : 'focus:ring-red-500'}`}
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                required
                                min="0"
                                className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 font-bold text-lg ${modalType === 'INCOME' ? 'focus:ring-green-500 text-green-600' : 'focus:ring-red-500 text-red-600'}`}
                                placeholder="0"
                        ...
                        {/* Material Purchase Logic - ONLY FOR EXPENSE + MATERIAL_PURCHASE */}
                        {modalType === 'EXPENSE' && formData.category === 'MATERIAL_PURCHASE' && (
                        ...
                        <div>
                            ...
                            <textarea 
                                rows={3}
                                className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 text-sm ${modalType === 'INCOME' ? 'focus:ring-green-500' : 'focus:ring-red-500'}`}
                 ...
                 <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 rounded-b-xl flex justify-end gap-3 shrink-0">
                    ...
                    <button 
                        ...
                        className={`px-6 py-2 text-white font-bold rounded-lg shadow-md transition-colors text-sm flex items-center ${modalType === 'INCOME' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}
                    >
